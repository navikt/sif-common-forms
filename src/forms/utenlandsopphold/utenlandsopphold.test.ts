...
const notIncludeInnlagtQuestion = false;
...
describe('utenlandsopphold', () => {
    it('maps utenlandsopphold to formValues correctly', () => {
        ...
        const mappedJson = jsonSort(mapUtenlandsoppholdToFormValues(utenlandsopphold, notIncludeInnlagtQuestion));
    ...
    it('maps formValues to utenlandsopphold correctly - with id', () => {
        ...
        const formJson = jsonSort(mapFormValuesToUtenlandsopphold(formValues, notIncludeInnlagtQuestion, id));
        expect(mappedJson).toEqual(formJson);
    });
